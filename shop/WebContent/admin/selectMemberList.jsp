	...
	<div class="container"> 
		...
		<div>	
				...
				<tbody>
					...
					%>
							<tr>
								...
								<td>
									<!-- 특정 회원정보를 삭제한다. -->
									<a class="btn btn-outline-dark" href="<%=request.getContextPath()%>/admin/deleteMemberForm.jsp?memberNo=<%=m.getMemberNo()%>&memberId=<%=m.getMemberId()%>&memberLevel=<%=m.getMemberLevel()%>&memberName=<%=m.getMemberName()%>&memberAge=<%=m.getMemberAge()%>&memberGender=<%=m.getMemberGender()%>">회원정보 탈퇴</a>
								</td>
							</tr>	
					<% 		
						}
			...
			<% 		
				...
				// [다음(>)] 버튼
				// 화면에 보여질 마지막 페이지 번호가 마지막 페이지 보다 작아지면 다음 버튼을 생성
				if(endPage < lastPage) {
			%>
